...
<p> The example shown below assumes the prior density is a mixture of two Gaussians with the same standard deviation &sigma;=0.15, the means &mu;<sub>1</sub> and &mu;<sub>2</sub>, and the probabilities P<sub>1</sub>=P(&mu;= &mu;<sub>1</sub>) and P<sub>2</sub>=1-P<sub>1</sub>.  The observation with (a usually high) probability P<sub>in</sub> is an inlier with a standard deviation of &sigma;<sub>y</sub>. Otherwise it is assumed to be an outlier, randomly lying in the interval [0,3]. </p>
